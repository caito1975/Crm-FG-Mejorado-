...
export default function DashboardClient({
  ...
  return (
    ...
    {showBrief && <BriefModal deals={deals} contacts={contacts} formatAmount={formatAmount} onClose={() => setShowBrief(false)} />}
    </>
  )
}
...
function BriefModal({ deals, contacts, formatAmount, onClose }: {
  deals: Deal[]
  contacts: Contact[]
  formatAmount: (v: number) => string
  onClose: () => void
}) {
  const props = deals.filter(d => d.stage_id === 'prop_enviada')
  const sin   = contacts.filter(c => !c.assigned_to && !c.owner_name)
        ...
        <div className="modal-body" style={{ overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-subtle)', marginBottom: 10 }}>
              Propuestas enviadas sin respuesta ({props.length})
            </div>
            {props.length === 0
